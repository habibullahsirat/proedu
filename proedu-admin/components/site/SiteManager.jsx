...
import { useSiteData } from "@/lib/DataFetch/SWRDataFetch";
import SiteForm from "@/components/site/SiteForm";
import Modal from "@/components/ui/Modal";
import { toast } from "sonner";

export default function SiteManager() {
  const { data: site, mutate, isLoading } = useSiteData();
  ...
  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this site data?")) return;
    ...
    try {
      ...
      toast.success(
        editingSite
          ...
          : "Site data added successfully!",
      );
      mutate(); // Refresh the data
      setIsModalOpen(false);
    } catch (error) {
      toast.error(error.message || "Failed to save site data");
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Total sites:{" "}
            <span className="font-semibold">{site?.length || 0}</span>
          </p>
        </div>
